...
library(tidyverse)
read.csv("CommQuest2023_Larger.csv")
```
...
```{r, eval=TRUE,echo=TRUE}
# Enter code here
comm_data %>% filter(channel == "Twitter", date == "2023-08-02") %>% select(date, channel, message)
```
<br>
...
```{r, eval=TRUE,echo=TRUE}
# Enter code here
positive_scores <- comm_data %>% filter(sentiment > 0)
average_scores <- positive_scores%>% group_by(sender) %>% summarise(average_sentiment = mean(sentiment))
sorted_scores <- average_scores %>% arrange(desc(average_sentiment))
top_three_senders <- sorted_scores %>% head(3) %>% select(sender, average_sentiment)
...
```{r, eval=TRUE,echo=TRUE}
# Enter code here
average_sentiment <- comm_data %>% group_by(date) %>% summarise(average_sentiment = mean(sentiment))
score_for_each_day <- arrange(average_sentiment, date)
print(score_for_each_day)
```
<br>
...
```{r, eval=TRUE,echo=TRUE}
# Enter code here
comm_data %>% mutate(sentiment) %>% filter(sentiment < 0) %>% select(message, sentiment)
```
<br>
...
```{r, eval=TRUE,echo=TRUE}
# Enter code here
comm_data %>% mutate(sentiment = case_when(sentiment > 0 ~ "Positive", sentiment < 0 ~ "Negative", TRUE ~ "Neutral"))
```
<br>
...
```{r, eval=TRUE,echo=TRUE}
# Enter code here
comm_data %>% mutate(message_length = nchar(message), product = sentiment * message_length) %>% arrange(desc(product))                      
```
<br>
...
**Solution:** Firstly, the dataset illustrated in slide 10 is non-tidy as there are multiple variables in the "subject" column. Secondly, the dataset contains the same variable in different format under the "Percent" column. It can be tidied using functions such as 'select' and 'arrange'.  